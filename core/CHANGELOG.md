## v0.27.0
- **New:** Added the `identity` package: a minimal `TokenProvider` contract with one provider per authentication flow (`StaticTokenProvider`, `ServiceAccountKeyProvider`, `WorkloadIdentityFederationProvider`, `InstanceMetadataProvider`), composable with `ChainedProvider`.
- **New:** Added `identity.NewDefaultProvider`, the opinionated credential chain: pre-issued token, service account key, workload identity federation, and instance metadata.
...
- **Deprecated:** The authentication entry points of `core/auth` (`SetupAuth`, `DefaultAuth`, `KeyAuth`, `TokenAuth`, `WorkloadIdentityFederationAuth`) and the flows of `core/clients` (`AuthFlow`, `KeyFlow`, `TokenFlow`, `WorkloadIdentityFederationFlow`, `TokenResponseBody`) in favour of the `identity` providers. `NoAuth` and `NoAuthFlow` are not deprecated: unauthenticated clients have no token provider equivalent.
