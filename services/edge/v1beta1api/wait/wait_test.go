...
func (s *mockSettings) getDeleteError() error {
	var err error
	if s.deleteCallCount < len(s.deleteErrors) {
		err = s.deleteErrors[s.deleteCallCount]
	}
	s.deleteCallCount++
	return err
}

func (s *mockSettings) getResourceError() error {
	if s.mockShouldTimeout {
		if len(s.resourceErrors) > 0 {
			return s.resourceErrors[0]
		}
		return nil
	}
	var err error
	if s.resourceCallCount < len(s.resourceErrors) {
		err = s.resourceErrors[s.resourceCallCount]
	}
	s.resourceCallCount++
	return err
}

func newAPIMock(settings *mockSettings) edge.DefaultAPI {
	getInstanceMock := func() (*edge.Instance, error) {
		if settings.deleteErrors != nil {
			err := settings.getDeleteError()
			if err != nil {
				return nil, err
			}
	...
	return &edge.DefaultAPIServiceMock{
		...
		GetInstanceByNameExecuteMock: utils.Ptr(func(_ edge.ApiGetInstanceByNameRequest) (*edge.Instance, error) {
			return getInstanceMock()
		}),
		GetKubeconfigByInstanceIdExecuteMock: utils.Ptr(func(_ edge.ApiGetKubeconfigByInstanceIdRequest) (*edge.Kubeconfig, error) {
			if err := settings.getResourceError(); err != nil {
				return nil, err
			}
			return &edge.Kubeconfig{}, nil
		}),
		GetKubeconfigByInstanceNameExecuteMock: utils.Ptr(func(_ edge.ApiGetKubeconfigByInstanceNameRequest) (*edge.Kubeconfig, error) {
			if err := settings.getResourceError(); err != nil {
				return nil, err
			}
			return &edge.Kubeconfig{}, nil
		}),
		GetTokenByInstanceIdExecuteMock: utils.Ptr(func(_ edge.ApiGetTokenByInstanceIdRequest) (*edge.Token, error) {
			if err := settings.getResourceError(); err != nil {
				return nil, err
			}
			return &edge.Token{}, nil
		}),
		GetTokenByInstanceNameExecuteMock: utils.Ptr(func(_ edge.ApiGetTokenByInstanceNameRequest) (*edge.Token, error) {
			if err := settings.getResourceError(); err != nil {
				return nil, err
			}
			return &edge.Token{}, nil
