...
func newAPIMock(settings *mockSettings) opensearch.DefaultAPI {
...
func TestCreateOrUpdateInstanceWaitHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			desc:          "succeeded",
			getFails:      false,
			resourceState: utils.Ptr(opensearch.INSTANCESTATUS_ACTIVE),
			wantErr:       false,
			wantResp:      true,
		},
		{
			desc:          "failed",
			getFails:      false,
			resourceState: utils.Ptr(opensearch.INSTANCESTATUS_FAILED),
			wantErr:       true,
			wantResp:      true,
		},
		{
		...
		},
		{
			desc:          "timeout",
			getFails:      false,
			resourceState: utils.Ptr(opensearch.InstanceStatus("ANOTHER_STATE")),
			wantErr:       true,
			wantResp:      false,
		},
	}

	handlers := map[string]func(context.Context, opensearch.DefaultAPI, string, string, string) *wait.AsyncActionHandler[opensearch.Instance]{
		...
		for _, tt := range tests {
			t.Run(fmt.Sprintf("%s - %s", handlerDesc, tt.desc), func(t *testing.T) {
				synctest.Test(t, func(t *testing.T) {
					...
					handler := handlerFn(context.Background(), apiClient, "pid", "eu01", instanceId)
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				handler := DeleteInstanceWaitHandler(context.Background(), apiClient, "", "eu01", instanceId)
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				handler := CreateCredentialsWaitHandler(context.Background(), apiClient, "", "eu01", "", credentialsId)
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				handler := DeleteCredentialsWaitHandler(context.Background(), apiClient, "", "eu01", "", credentialsId)
