...
func newAPIMock(settings *mockSettings) mariadb.DefaultAPI {
	return &mariadb.DefaultAPIServiceMock{
		...
		GetCredentialsExecuteMock: utils.Ptr(func(_ mariadb.ApiGetCredentialsRequest) (*mariadb.CredentialsResponse, error) {
			...
			return &mariadb.CredentialsResponse{
...
func TestCreateOrUpdateInstanceWaitHandler(t *testing.T) {
	...
	handlers := map[string]func(context.Context, mariadb.DefaultAPI, string, string, string) *wait.AsyncActionHandler[mariadb.Instance]{
...
func TestCreateCredentialsWaitHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			var wantRes *mariadb.CredentialsResponse
			if tt.wantResp {
				wantRes = &mariadb.CredentialsResponse{
					Id: credentialsId,
				}
			}
