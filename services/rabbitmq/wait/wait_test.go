...
type apiClientInstanceMocked struct {
	...
	resourceId                 string
	resourceOperation          *rabbitmq.InstanceLastOperationTypes
...
func (a *apiClientInstanceMocked) GetInstanceExecute(_ context.Context, _, _ string) (*rabbitmq.Instance, error) {
	...
	if a.resourceOperation != nil && *a.resourceOperation == rabbitmq.INSTANCELASTOPERATIONTYPE_DELETE && a.resourceState == rabbitmq.INSTANCESTATUS_ACTIVE {
...
func (a *apiClientCredentialsMocked) GetCredentialsExecute(_ context.Context, _, _, _ string) (*rabbitmq.CredentialsResponse, error) {
	if a.getFails {
		return nil, &oapierror.GenericOpenAPIError{
			StatusCode: 500,
		}
	...
	return &rabbitmq.CredentialsResponse{
		Id: &a.resourceId,
	}, nil
}
...
func TestCreateInstanceWaitHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			getFails:      false,
			resourceState: rabbitmq.INSTANCESTATUS_ACTIVE,
			wantErr:       false,
			wantResp:      true,
		},
		{
			desc:          "create_failed",
			getFails:      false,
			resourceState: rabbitmq.INSTANCESTATUS_FAILED,
			wantErr:       true,
			wantResp:      true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			apiClient := &apiClientInstanceMocked{
				...
				resourceId:                 instanceId,
				resourceOperation:          utils.Ptr(rabbitmq.INSTANCELASTOPERATIONTYPE_DELETE),
