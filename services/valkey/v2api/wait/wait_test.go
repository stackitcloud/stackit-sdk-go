...
type mockSettings struct {
	...
	instanceResourceOperation          valkey.InstanceLastOperationType
	instanceResourceState              *valkey.InstanceStatus
...
func newAPIMock(settings *mockSettings) valkey.DefaultAPI {
	return &valkey.DefaultAPIServiceMock{
		GetInstanceExecuteMock: utils.Ptr(func(_ valkey.ApiGetInstanceRequest) (*valkey.Instance, error) {
			if settings.instanceGetFails {
				return nil, &oapierror.GenericOpenAPIError{
					StatusCode: 500,
				}
			}
			if settings.instanceResourceOperation == valkey.INSTANCELASTOPERATIONTYPE_DELETE && settings.instanceResourceState != nil && *settings.instanceResourceState == valkey.INSTANCESTATUS_ACTIVE {
		...
		GetCredentialsExecuteMock: utils.Ptr(func(_ valkey.ApiGetCredentialsRequest) (*valkey.CredentialsResponse, error) {
			...
			return &valkey.CredentialsResponse{
...
func TestCreateOrUpdateInstanceWaitHandler(t *testing.T) {
	tests := []struct {
		desc          string
		getFails      bool
		resourceState *valkey.InstanceStatus
		wantErr       bool
		wantResp      bool
	}{
		{
			...
			getFails:      false,
			resourceState: utils.Ptr(valkey.INSTANCESTATUS_ACTIVE),
			wantErr:       false,
			wantResp:      true,
		},
		{
			...
			getFails:      false,
			resourceState: utils.Ptr(valkey.INSTANCESTATUS_FAILED),
			wantErr:       true,
			wantResp:      true,
		},
		{
			...
			getFails:      false,
			resourceState: utils.Ptr(valkey.InstanceStatus("ANOTHER STATE")),
			wantErr:       true,
			wantResp:      false,
		},
		{
	...
	handlers := map[string]func(context.Context, valkey.DefaultAPI, string, string, string) *wait.AsyncActionHandler[valkey.Instance]{
		...
		for _, tt := range tests {
			t.Run(fmt.Sprintf("%s - %s", handlerDesc, tt.desc), func(t *testing.T) {
				synctest.Test(t, func(t *testing.T) {
					...
					var wantRes *valkey.Instance
					if tt.wantResp {
						wantRes = &valkey.Instance{
							InstanceId: &instanceId,
							Status:     tt.resourceState,
						}
					}

					handler := handlerFn(context.Background(), apiClient, "pid", testRegion, instanceId)
		...
		t.Run(tt.desc, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				apiClient := newAPIMock(&mockSettings{
					...
					instanceResourceId:                 instanceId,
					instanceResourceOperation:          valkey.INSTANCELASTOPERATIONTYPE_DELETE,
					instanceResourceDescription:        tt.resourceDescription,
					instanceResourceState:              tt.resourceState,
				})

				handler := DeleteInstanceWaitHandler(context.Background(), apiClient, "", testRegion, instanceId)
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				var wantRes *valkey.CredentialsResponse
				if tt.wantResp {
					wantRes = &valkey.CredentialsResponse{
						Id: credentialsId,
					}
				}

				handler := CreateCredentialsWaitHandler(context.Background(), apiClient, "", testRegion, "", credentialsId)
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				handler := DeleteCredentialsWaitHandler(context.Background(), apiClient, "", testRegion, "", credentialsId)
