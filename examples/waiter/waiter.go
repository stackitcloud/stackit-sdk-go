...
func main() {
	...
	createZoneResp, err := dnsClient.DefaultAPI.CreateZone(ctx, projectId).CreateZonePayload(createZonePayload).Execute()
	if err != nil {
	...
	wres, err := wait.CreateZoneWaitHandler(ctx, dnsClient.DefaultAPI, projectId, zoneId).SetTimeout(15 * time.Minute).WaitWithContext(ctx)
	if err != nil {
