...
func main() {
	...
	addRoutingTableToAreaPayload := iaas.AddRoutingTableToAreaPayload{
		Name:        "example-routing-tables",
		Description: utils.Ptr("example"),
	}

	routingTable, err := iaasClient.DefaultAPI.AddRoutingTableToArea(context.Background(), organizationId, networkAreaId, region).AddRoutingTableToAreaPayload(addRoutingTableToAreaPayload).Execute()
	if err != nil {
	...
	updatedRoutingTable, err := iaasClient.DefaultAPI.UpdateRoutingTableOfArea(context.Background(), organizationId, networkAreaId, region, *routingTable.Id).UpdateRoutingTableOfAreaPayload(updateRoutingTableOfAreaPayload).Execute()
	if err != nil {
	...
	route, err := iaasClient.DefaultAPI.AddRoutesToRoutingTable(context.Background(), organizationId, networkAreaId, region, *updatedRoutingTable.Id).AddRoutesToRoutingTablePayload(addRoutesToRoutingTablePayload).Execute()
	if err != nil {
	...
	err = iaasClient.DefaultAPI.DeleteRouteFromRoutingTable(context.Background(), organizationId, networkAreaId, region, *updatedRoutingTable.Id, *(route.Items)[0].Id).Execute()
	if err != nil {
	...
	err = iaasClient.DefaultAPI.DeleteRoutingTableFromArea(context.Background(), organizationId, networkAreaId, region, *updatedRoutingTable.Id).Execute()
	if err != nil {
